...
import { CombiCalculated, MahjongScoring, NumeroVent } from "../dataModel/dataUtils";
import { CombiScoringRule, MahjongScoringRule } from "../rules/interfacesScoringRules";
import { getCombiScoringRules, getMahjongScoringRules } from "../rules/readScoringRules";
 ...
 */
function getCombiScoringRulesFromCombinaison(combinaison : CombiCalculated) : CombiScoringRule | undefined {
    ...
    for(let i = 0 ; i < combiScoringRules.length ; i++){
        ...
        if(combiScoringRule.baseCombi === combinaison.base && compareSet(new Set(combiScoringRule.modificateur), combinaison.modificateur)){
 ...
 */
function getMahjongScoringRulesFromId(id : number) : MahjongScoringRule | undefined {
    // get all combinaison rules
...
export function calculateCombiScore(combinaisons : Combinaison[], joueurVent : NumeroVent, dominant : NumeroVent) : CombiScore {
    ...
    for(let i = 0 ; i < combinaisons.length ; i++){
        ...
        if(combi !== undefined){
            const combiScoringRule : CombiScoringRule | undefined = getCombiScoringRulesFromCombinaison(combi);
 ...
 */
export function calculateScoring(combinaisons : Combinaison[], joueurVent : NumeroVent, dominant : NumeroVent) : number {
    // get the mahjong scoring of the combinaisons
    const mahjongScoring : MahjongScoring[] = getMahjongScoring(combinaisons, joueurVent, dominant);
    ...
    for(let i = 0 ; i < mahjongScoring.length ; i++){
        const combiMahjongScoringRule : MahjongScoringRule | undefined = getMahjongScoringRulesFromId(mahjongScoring[i]);
